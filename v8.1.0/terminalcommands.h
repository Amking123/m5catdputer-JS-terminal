...
#include "menu.h"
#include "bios.h"
#include "sharescreen_spi.h" // Include your new header for SPI sharing

// Forward declarations
void registerJSBindings(duk_context *ctx);

// Declare the global flag from terminalmain.ino
...
void processCommand(duk_context *ctx, String input) {
  ...
  if (input.equalsIgnoreCase("help")) {
    shellPrint("Commands: help, clear, menu, sharescreen, about, bios, reboot, random, run <app>");
    shellPrint("JS: print(), load(), run(), require(), getKey(), etc.");
  }
  else if (input.equalsIgnoreCase("clear")) {
    if (shareScreenModeActive) {
      sendShareScreenClear();
    }
    M5Cardputer.Display.fillScreen(BLACK);
    outputLine = 0;
  }
  else if (input.equalsIgnoreCase("sharescreen")) {
    shareScreenModeActive = !shareScreenModeActive;
    if (shareScreenModeActive) {
      shellPrint("ShareScreen mode active. Output will be sent via SPI.", BLUE);
      sendShareScreenClear();
    } else {
      shellPrint("ShareScreen mode inactive. Output returns to local display.", BLUE);
    }
  }
   ...
   shellPrint("║ Version 8.1.0 sharescreen ║", CYAN);
   shellPrint("╚═══════════════════════════╝", CYAN);
  }
...
static duk_ret_t js_print(duk_context *ctx) {
  ...
  // Conditionally send output to SPI or the local shell
  if (shareScreenModeActive) {
    sendShareScreenText(out.c_str());
  } else {
    shellPrint(out);
  }
  return 0;
